...
class TencentBaseUploader(BaseVideoUploader):
    ...
    async def set_schedule_time_tencent(self, page: Page, publish_date: datetime, keyboard_page: Page | None = None):
        keyboard = (keyboard_page or page).keyboard
        ...
        await keyboard.press("Control+KeyA")
        await keyboard.type(publish_date.strftime("%H"))
    ...
    async def open_upload_page(self, page: Page) -> None:
        ...
        for attempt in range(2):
            try:
                await page.wait_for_function(
                    """() => /上传时长|视频描述|短标题|发表|保存草稿/.test(document.body?.innerText || '')""",
                    timeout=30000,
                )
                return
            except Exception:
        ...
        page_text = ""
        try:
            page_text = (await page.locator("body").inner_text(timeout=3000)).strip().replace("\n", " ")[:1000]
        except Exception:
            pass
        raise RuntimeError(f"视频号发布表单没有加载出来，当前页面文本: {page_text}")

    async def publish_frame(self, page: Page) -> Page:
        if not hasattr(page, "frames"):
            return page
        for frame in page.frames:
            try:
                if "/micro/content/post/create" in frame.url:
                    return frame
            except Exception:
                continue
        return page
    ...
    async def upload_video_file(self, page: Page, file_path: str) -> None:
        target_page = await self.publish_frame(page)
        ...
        upload_targets = [
            target_page.locator('div:has-text("上传时长8小时内"):has-text("MP4")').first,
            target_page.locator('div:has-text("上传时长"):has-text("20GB")').first,
            target_page.locator('div:has-text("MP4/H.264格式")').first,
            target_page.locator('div:has-text("MP4")').first,
        ]
        last_error = None
        ...
        text = ""
        try:
            text = (await target_page.locator("body").inner_text(timeout=3000)).strip().replace("\n", " ")[:1000]
        except Exception:
            pass
        raise RuntimeError(f"未找到视频号视频上传入口: {last_error or 'no file input/file chooser'}; 页面文本: {text}")
    ...
    async def fill_title_and_tags(self, page: Page, keyboard_page: Page | None = None) -> None:
        keyboard = (keyboard_page or page).keyboard
        await page.locator("div.input-editor").click()
        await keyboard.type(self.title)
        await keyboard.press("Enter")
        for tag in self.tags:
            await keyboard.type("#" + tag)
            await keyboard.press("Space")
        tencent_logger.info(_msg("🏷️", f"成功添加 hashtag: {len(self.tags)}"))

    async def fill_description(self, page: Page, keyboard_page: Page | None = None) -> None:
        keyboard = (keyboard_page or page).keyboard
        await keyboard.press("Enter")
        await keyboard.type(self.desc)
...
class TencentVideo(TencentBaseUploader):
    ...
    async def prepare_video_for_publish(self, page: Page, keyboard_page: Page | None = None) -> None:
        await self.fill_title_and_tags(page, keyboard_page=keyboard_page)
        await self.fill_description(page, keyboard_page=keyboard_page)
    ...
    async def upload(self, playwright: Playwright) -> None:
        ...
        try:
            ...
            publish_page = await self.publish_frame(page)
            tencent_logger.info(_msg("🏃", f"小人开始搬运视频: {self.title}"))

            await self.upload_video_file(page, self.file_path)
            await self.prepare_video_for_publish(publish_page, keyboard_page=page)
            await self.wait_for_upload_complete(publish_page)
            await self.set_thumbnail(publish_page)

            if self.publish_strategy == TENCENT_PUBLISH_STRATEGY_SCHEDULED and self.publish_date != 0:
                await self.set_schedule_time_tencent(publish_page, self.publish_date, keyboard_page=page)

            await self.set_short_title(publish_page, self.title, self.short_title)
            await self.submit_publish(publish_page)
