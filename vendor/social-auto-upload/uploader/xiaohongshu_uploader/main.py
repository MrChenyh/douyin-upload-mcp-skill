...
class XiaoHongShuBaseUploader(BaseVideoUploader):
    ...
    async def desc_editor(self, page: Page):
        candidates = [
            'p[data-placeholder*="输入正文描述"]',
            '[contenteditable="true"][data-placeholder*="输入正文描述"]',
            '[contenteditable="true"][placeholder*="输入正文描述"]',
            'div[contenteditable="true"]:has-text("")',
        ...
        for selector in candidates:
            ...
            try:
                ...
                continue
        raise RuntimeError("xiaohongshu_publish_form_desc_editor_not_found")

    async def fill_desc(self, page: Page) -> None:
        if not getattr(self, "desc", ""):
            return

        desc = await self.desc_editor(page)
        await desc.click()
    ...
    async def fill_tags(self, page: Page) -> None:
        ...
        for tag in self.tags:  # 循环处理所有 tags
            ...
            try:
                ...
                xiaohongshu_logger.warning(_msg("😵", f"话题下拉没有出现，保留正文标签继续发布: #{cleaned} ({exc})"))
