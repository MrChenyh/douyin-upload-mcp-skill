...
class BrowserCliParserTests(unittest.TestCase):
    ...
    def test_tencent_upload_video_accepts_thumbnail_and_short_title(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            video_path = Path(tmp_dir) / "demo.mp4"
            video_path.write_bytes(b"video")
        ...
        self.assertEqual(args.desc, "视频号简介")
        self.assertEqual(args.short_title, "短标题")
...
class BrowserCliDispatchTests(unittest.TestCase):
    ...
    def test_dispatch_tencent_upload_video_uses_request_fields(self):
        args = Namespace(
            platform="tencent",
            action="upload-video",
            account="creator",
        ...
        self.assertEqual(request.tags, ["测试", "视频号"])
        self.assertEqual(request.short_title, "短标题")
        self.assertTrue(request.headless)
