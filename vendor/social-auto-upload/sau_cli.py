...
def local_app_data_dir() -> Path:
    root = os.environ.get("LOCALAPPDATA") or os.environ.get("TMP") or os.environ.get("TEMP")
    if root:
        return Path(root) / "DouyinPublisherDesktopMMVP"
    return resolve_runtime_home() / ".runtime"
...
def write_image_unicode(path: Path, image) -> None:
    ...
    ok, encoded = cv2.imencode(path.suffix or ".jpg", image, [int(cv2.IMWRITE_JPEG_QUALITY), 94])
    if not ok:
        raise RuntimeError(f"failed_to_encode_image:{path}")
    path.write_bytes(encoded.tobytes())


def prepare_douyin_landscape_thumbnail(thumbnail_file: Path | None) -> Path | None:
    if not thumbnail_file:
        return None

    source = Path(thumbnail_file).expanduser().resolve()
    try:
        import cv2
        import numpy as np

        image = read_image_unicode(source)
        if image is None:
            return source
        ...
        digest = hashlib.sha1(f"{source}:{source.stat().st_mtime_ns}:{source.stat().st_size}".encode("utf-8")).hexdigest()[:12]
        output = local_app_data_dir() / "social-auto-upload" / "covers" / f"{source.stem}-{digest}-landscape.jpg"
        if output.exists() and output.stat().st_size > 0:
            return output

        background_scale = max(target_width / width, target_height / height)
        background_size = (max(1, int(width * background_scale)), max(1, int(height * background_scale)))
        background = cv2.resize(image, background_size, interpolation=cv2.INTER_AREA)
        ...
        background = cv2.addWeighted(background, 0.55, np.zeros_like(background), 0.45, 0)

        foreground_scale = min(target_width / width, target_height / height)
        foreground_size = (max(1, int(width * foreground_scale)), max(1, int(height * foreground_scale)))
        foreground = cv2.resize(image, foreground_size, interpolation=cv2.INTER_AREA)
        offset_x = (target_width - foreground.shape[1]) // 2
        offset_y = (target_height - foreground.shape[0]) // 2
        background[offset_y:offset_y + foreground.shape[0], offset_x:offset_x + foreground.shape[1]] = foreground

        write_image_unicode(output, background)
        return output
    except Exception as exc:
        print(f"warning: failed to prepare Douyin landscape thumbnail, fallback to original: {exc}", file=sys.stderr)
        return source
...
async def upload_video(request: DouyinVideoUploadRequest) -> Path:
    ...
    thumbnail_portrait_file = request.thumbnail_portrait_file or request.thumbnail_file
    thumbnail_landscape_file = request.thumbnail_landscape_file or prepare_douyin_landscape_thumbnail(thumbnail_portrait_file)
    ...
    app = DouYinVideo(
        ...
        thumbnail_landscape_path=str(thumbnail_landscape_file) if thumbnail_landscape_file else None,
        thumbnail_portrait_path=str(thumbnail_portrait_file) if thumbnail_portrait_file else None,
...
async def upload_tencent_video(request: TencentVideoUploadRequest) -> Path:
    ...
    app = TencentVideo(
        ...
        desc=request.description,
        thumbnail_path=str(request.thumbnail_file) if request.thumbnail_file else None,
        short_title=request.short_title or None,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    upload_video_parser.add_argument("--thumbnail-landscape", type=existing_file_path, help="Optional 16:9 thumbnail path for Douyin works list")
    upload_video_parser.add_argument("--thumbnail-portrait", type=existing_file_path, help="Optional 9:16 thumbnail path for Douyin mobile cover")
    ...
    tencent_parser = platform_parsers.add_parser("tencent", help="WeChat Channels operations")
    tencent_actions = tencent_parser.add_subparsers(dest="action", required=True)

    for action_name in ("login", "check"):
        action_parser = tencent_actions.add_parser(action_name, help=f"WeChat Channels {action_name}")
        action_parser.add_argument("--account", required=True, help="WeChat Channels user-defined account_name")
        if action_name == "login":
            add_runtime_flags(action_parser)

    tencent_upload_video_parser = tencent_actions.add_parser("upload-video", help="Upload one video to WeChat Channels")
    tencent_upload_video_parser.add_argument("--account", required=True, help="WeChat Channels user-defined account_name")
    ...
    tencent_upload_video_parser.add_argument("--short-title", default="", help="Optional short title for profile card")
...
async def dispatch(args: argparse.Namespace) -> int:
    ...
    if args.platform == "tencent":
        if args.action == "login":
            ...
            if not result["success"]:
                raise RuntimeError(result["message"])
            print(f"WeChat Channels login flow completed: {result['account_file']}")
            return 0
        ...
        if args.action == "upload-video":
            ...
            print(f"WeChat Channels video upload submitted: {request.video_file}")
            return 0

        raise RuntimeError(f"Unsupported WeChat Channels action: {args.action}")
