 ...
 *   2. .env.local（本机真实测试/密钥配置，git-ignored）
 *   3. .env.development（开发环境，git-ignored）
 *   4. .env（基础配置，可提交到 git）
 *   5. 代码默认值
 */
...
function envIntMin(key, fallback, minimum) {
  return Math.max(envInt(key, fallback), minimum);
}
