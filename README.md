...
仓库已经包含 `vendor/xiaoice-video-tool` 的可分发代码。安装后需要在目标机器本地复制 `templates/skill-local-config.sample` 或 `.env.example` 为 `.env.local`，并填写自己的飞书、模型、多维表、Coze/小冰配置；`bootstrap-openclaw.js --apply` 会把 vendor 工具安装到 `~/自动营销/xiaoice-video-tool`，并从 `templates/skill-local-config.sample` 或 `.env.example` 生成小冰工具 `.env` 模板。
